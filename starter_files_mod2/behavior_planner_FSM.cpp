...
State BehaviorPlannerFSM::state_transition(
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    // Get the previous goal state
    goal = _goal;
    
    // Since we don't have a motion controller yet), use "distance" instead 
    // of speed. Make sure the distance to the stopping point is <=
    ...
    // Keep previous goal. Stay where you are.
    goal = _goal;
    ...
    if (stopped_secs >= _req_stop_time && tl_state.compare("Red") != 0) {
      // Move to FOLLOW_LANE state: Assign the state we want to move to, when
