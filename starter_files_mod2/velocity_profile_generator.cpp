...
double VelocityProfileGenerator::calc_distance(
  ...
  if (std::abs(a) < DBL_EPSILON) {
    // Divide-by-zero case - return INF if acceleration is below a small threshold
    d = std::numeric_limits<double>::infinity();
  }
  else {
    // Compute the distance based on acceleration and velocity ( 1D rectilinear motion equation )
...
double VelocityProfileGenerator::calc_final_speed(
  ...
  d_inf = std::numeric_limits<double>::infinity();

  // Based on 1D rectilinear motion equation for velocity, compute the final speed
  double disc = std::pow(v_i, 2) + 2 * a * d;

  if (disc <= 0.0) {
    v_f = 0.0;
  }
  else if (disc == d_inf || std::isnan(disc)
  ) {
    v_f = d_inf;
  }
  else {
